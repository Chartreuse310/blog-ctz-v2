...
4. [ ] 编写文章正文，在正文中引用图片
5. [ ] `npm run docs:dev` 本地预览验证（侧边栏需重启刷新）
6. [ ] 检查侧边栏是否正确显示新文章（日期胶囊 + 简化标题）
7. [ ] `npm run docs:build` 构建验证
8. [ ] 提交代码并部署
...
侧边栏会自动扫描 `docs/posts/` 目录生成文章列表，无需手动配置。

### 显示规则

- **标题简化**：标题中的「——」及之后的内容会被自动截取，只显示主标题部分
  - 原文：`关于我的最终版（×7.1）知识组织方法——Solution Z的第一版设计`
  - 侧边栏显示：`关于我的最终版（×7.1）知识组织方法`
- **日期胶囊**：文章标题前会显示日期小胶囊（如 `[2026-07-03]`）
- **排序方式**：按日期降序排列，最新文章显示在最上方

### 交互行为

- 点击「近期文章」可跳转回文章列表页 `/posts/`
- 「近期文章」下方有分割线与文章列表分隔

### 更新注意事项

- 新增文章后，需要重启 `npm run docs:dev` 才能刷新侧边栏
- 修改文章标题或日期后，同样需要重启 dev server 才能生效
